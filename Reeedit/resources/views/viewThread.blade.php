        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
          {{ $data["thread"]->description }}
            ...
            <div class="row"  style="margin: 10px">
                ...
                @else
                  <a href="{{ $data['thread']->id }}/tvoteup"><i class="material-icons tiny grey-text">thumb_up</i></a>
                @endif
                <b> {{ (($data['thread']->votes > 0)? "+" : "") }}{{ $data['thread']->votes }} </b>
                @if( $data['thread']->myVote == -1)
                  <a href="#"><i class="material-icons tiny red-text">thumb_down</i></a>
                @else
                  <a href="{{ $data['thread']->id }}/tvotedown"><i class="material-icons tiny grey-text">thumb_down</i></a>
                @endif
                </div>
                <div class="col s6 right-align">
                  By <b>{{$data['thread']->author}} </b> on {{$data['thread']->createdAt}}
                </div>
            </div>
          ...
          @endif
            ...
            <div class="row"  style="margin: 5px">
                ...
                @if( $answer['data']->myVote == 1)
                  <a href="#"><i class="material-icons tiny green-text">thumb_up</i></a>
                @else
                  <a href="{{ $data['thread']->id }}/voteup/{{ $answer['data']->id }}"><i class="material-icons tiny grey-text">thumb_up</i></a>
                @endif
                <b> {{ (($answer['data']->votes > 0)? "+" : "") }}{{ $answer['data']->votes }} </b>
                @if( $answer['data']->myVote == -1)
                  <a href="#"><i class="material-icons tiny red-text">thumb_down</i></a>
                @else
                  <a href="{{ $data['thread']->id }}/votedown/{{ $answer['data']->id }}"><i class="material-icons tiny grey-text">thumb_down</i></a>
                @endif
                </div>
                <div class="col s6 right-align">
